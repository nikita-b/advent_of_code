from collections import defaultdict

if __name__ == '__main__':
    contents = """\
Card 1: 41 48 83 86 17 | 83 86  6 31 17  9 48 53
Card 2: 13 32 20 16 61 | 61 30 68 82 17 32 24 19
Card 3:  1 21 53 59 44 | 69 82 63 72 16 21 14  1
Card 4: 41 92 73 84 69 | 59 84 76 51 58  5 54 83
Card 5: 87 83 26 28 32 | 88 30 70 12 93 22 82 36
Card 6: 31 18 13 56 72 | 74 77 10 23 35 67 36 11"""
    contents = open("day4-input.txt", "r").read()
    total_score = 0
    copies = defaultdict(lambda: 1)
    for card in contents.split("\n"):
        card_number, data = card.split(":")
        card_number = int(card_number.split()[1])

        scratchcards, winning = data.split("|")
        winning_set = set()
        for w in winning.split():
            winning_set.add(w)
        score = 0
        for s in scratchcards.split():
            if s in winning_set:
                score += 1
        for j in range(copies[card_number]):
            for i in range(card_number + 1, card_number + score + 1):
                copies[i] += 1

    result = 0
    for card, number in copies.items():
        result += number

    print("RESULT: ", result)


